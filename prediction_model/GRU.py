...
class transformers(nn.Module):
    def __init__(self,in_feature,out_feature):
        ...
        self.gru = nn.GRU(input_size=11, hidden_size=seq_len,num_layers=2,batch_first=True,bidirectional=False)
...
_model = torch.load('./model/model_gru.pt')#18
    ...
    with torch.no_grad():
        ...
        for date_item in dataset:
            ...
            if len(all_data_true)%10==0:
                plt.plot(np.arange(0,len(all_data_true)),list(np.array(scalerfenchen.inverse_transform([all_data_true[i : i + 10] for i in range(0, len(all_data_true), 10)])).ravel()),'rs:',label='pred value')
