...
class transformers(nn.Module):
    def __init__(self,in_feature,out_feature):
        ...
        self.lstm = nn.LSTM(input_size=11, hidden_size=seq_len,num_layers=2,batch_first=True,bidirectional=False)
...
_model = torch.load('./model/model_lstm.pt')
total_params = sum(p.numel() for p in _model.parameters())
total_params += sum(p.numel() for p in _model.buffers())
...
def predict(model, dataset,scalerfenchen):
    ...
    with torch.no_grad():
        ...
        for date_item in dataset:
            ...
            plt.title('true value and  pred value')
        ...
        data_result_write = pd.ExcelWriter("./lstm_predict_result.xlsx")
